...
function AppInner() {
  ...
  // ── 0.5. Profile Completion (placeholder – implement as needed) ───────────────────────────────────────────────────
  ...
  const renderView = () => {
    ...
    if (role === 'farmer') {
      switch (currentView) {
        case 'farmer':
          return <FarmerDashboard onNavigate={handleNavigate} onLogout={handleLogout} />;
        case 'home':
    ...
    if (role === 'buyer') {
      switch (currentView) {
        case 'buyer':
          return <BuyerDashboard onNavigate={handleNavigate} onLogout={handleLogout} />;
        case 'home':
          return <HomeScreen onNavigate={handleNavigate} />;
        default:
          return <BuyerDashboard onNavigate={handleNavigate} onLogout={handleLogout} />;
    ...
    if (role === 'transport') {
      switch (currentView) {
        case 'transport':
          return <TransportDashboard onNavigate={handleNavigate} onLogout={handleLogout} />;
        case 'home':
          return <HomeScreen onNavigate={handleNavigate} />;
        default:
          return <TransportDashboard onNavigate={handleNavigate} onLogout={handleLogout} />;
  ...
  // Wrap view in Suspense with a styled fallback to avoid white flash
  return (
    <Suspense fallback={<div className="fixed inset-0 bg-gradient-to-br from-gold-200 via-gold-300 to-sunset-300 z-50" />}> 
      {renderView()}
    </Suspense>
  );
}
